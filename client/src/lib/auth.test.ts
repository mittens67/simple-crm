...
import { has_permission } from './auth';

describe('has_permission utility', () => {
  ...
  it('returns true for global wildcard permission', () => {
    ...
    expect(has_permission(permissions, 'anything')).toBe(true);
  });
  ...
  it('works with multiple resources', () => {
    const permissions = {
      ...
      'customers.*': true,
      'deals.delete': false,
      '*': false, // Global deny is overridden by specific grants
    };
